...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private double theta = 2 * Math.PI / sides;

	private void regularPyramidBase(GL2 gl2) {
		double x, y;
		gl2.glBegin(GL.GL_TRIANGLE_FAN);
		gl2.glTexCoord3d(0, 0, 0);
		for (int i = 0; i < sides + 1; i++) {
			x = Math.cos(theta * i);
			y = Math.sin(theta * i);
			gl2.glVertex3d(x, y, 1);
			gl2.glTexCoord3d(x, y, 1);
		}
		gl2.glEnd();
	}

	private void regularPyramidWalls(GL2 gl2) {
		double x, y;
		gl2.glBegin(GL.GL_TRIANGLE_FAN);
	...
	private void regularPyramid(GL2 gl2) {
		gl2.glScalef(0.5f, 0.5f, 0.5f);

		regularPyramidBase(gl2);
		regularPyramidWalls(gl2);
	}
	...
	private Texture textureFromResource(String resourceName) throws IOException {
		...
		Texture texture = AWTTextureIO.newTexture(displayGL.getGLProfile(), image, true);
	...
	public JMenuBar getMenuBar() {
		...
		makeMenuItem(objectMenu, "Pyramide", objectListener, 7);
